...
test('@types/set-of-type', () => {
  test('validate', () => {
    should('should pass if all the elements passes the given type', () => {
      assert(
        setOfType(stringType).validate(new Set(['a', 'b', 'c'])),
      ).to.haveProperties({
        passes: true,
      });
    });
    ...
    should(
      'should not pass if an element in the set does not pass the given type',
      () => {
        assert(
          setOfType(stringType).validate(new Set(['a', 1, 'c'])),
        ).to.haveProperties({
