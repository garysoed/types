...
test('@types/map-of-type', () => {
  test('validate', () => {
    should('should pass if all the elements passes the given type', () => {
      const result = mapOfType(numberType, stringType).validate(new Map([
        [1, 'a'],
        [2, 'b'],
        [3, 'c'],
      ]));
    ...
    should('should not pass if a key in the map does not pass the given type', () => {
      const result = mapOfType(numberType, stringType).validate(new Map<unknown, string>([
        [1, 'a'],
        ['2', 'b'],
        [3, 'c'],
      ]));
    ...
    should('should not pass if a value in the map does not pass the given type', () => {
      const result = mapOfType(numberType, stringType).validate(new Map<number, unknown>([
        [1, 'a'],
        [2, 123],
        [3, 'c'],
      ]));
